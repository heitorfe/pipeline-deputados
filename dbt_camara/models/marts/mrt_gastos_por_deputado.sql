...
WITH gastos_deputado AS (
    SELECT 
        ...
        COUNT(DISTINCT CONCAT(fd.ano, '-', LPAD(fd.mes, 2, '0'))) AS total_meses_atividade,
        ...
        ROUND(SUM(CASE WHEN fd.faixa_valor = 'Médio' THEN fd.valor_documento ELSE 0 END), 2) AS gastos_valor_medio,
        ...
        ROUND(SUM(fd.valor_liquido) / (30416.83 * COUNT(DISTINCT CONCAT(fd.ano, '-', LPAD(fd.mes, 2, '0')))) * 100, 2) AS percentual_uso_cota_mensal
        
    FROM {{ ref('fct_despesas') }} fd
    INNER JOIN {{ ref('dim_deputados') }} dd 
        ON fd.sk_deputado = dd.sk_deputado
